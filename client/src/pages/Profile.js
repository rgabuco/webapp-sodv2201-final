...
function Profile() {
    ...
    const handleFileChange = async event => {
        ...
        try {
            const response = await axios.post(`http://localhost:5000/api/v1/users/${loggedInUser._id}/profile-photo`, formData, {
                headers: {
    ...
    const handleSave = async () => {
        ...
        try {
            if (Object.keys(updateData).length > 0) {
                const response = await axios.patch(`${process.env.REACT_APP_SERVER_URL}/api/v1/users/${loggedInUser._id}`, updateData, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`,
                    },
                });
