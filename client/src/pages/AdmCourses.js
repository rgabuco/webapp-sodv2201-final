...
function AdmCourses() {
        ...
        const fetchCourses = async () => {
            try {
                ...
                const transformedCourses = programs.reduce((acc, program) => {
                    acc[program.name] = program.courses.map(course => ({
                        ...course,
    ...
    const handleSaveEdit = async () => {
        try {
            ...
            await axios.patch(`${process.env.REACT_APP_SERVER_URL}/api/v1/courses/${editCourse._id}`, editCourse, config);

            const updatedCourses = { ...courses };
            for (const program in updatedCourses) {
                updatedCourses[program] = updatedCourses[program].map(c => (c._id === editCourse._id ? editCourse : c));
            }
    ...
    const handleConfirmDelete = async () => {
        try {
            ...
            await axios.delete(`${process.env.REACT_APP_SERVER_URL}/api/v1/courses/${deleteCourse._id}`, config);

            const updatedCourses = { ...courses };
            for (const program in updatedCourses) {
                updatedCourses[program] = updatedCourses[program].filter(c => c._id !== deleteCourse._id);
            }
            setCourses(updatedCourses);
        } catch (error) {
                ...
                <Dialog open={true} onClose={() => setEditCourse(null)}>
                    ...
                    <DialogContent>
                        ...
                        <TextField
                            ...
                            fullWidth
                            margin="normal"
                            value={editCourse ? formatDate(editCourse.startDate) : ""}
                        ...
                        <TextField
                            ...
                            fullWidth
                            margin="normal"
                            value={editCourse ? formatDate(editCourse.endDate) : ""}
                        ...
                        <FormControl fullWidth margin="normal" variant="outlined">
                            ...
                            <Select
                                value={editCourse.deliveryMode}
                                onChange={e => setEditCourse({ ...editCourse, deliveryMode: e.target.value })}
                                label="Delivery Mode"
                            >
                        ...
                        <TextField
                            ...
                            fullWidth
                            margin="normal"
                            value={editCourse.classSize}
                            onChange={e => setEditCourse({ ...editCourse, classSize: e.target.value })}
                        />
                    </DialogContent>
                    <DialogActions>
